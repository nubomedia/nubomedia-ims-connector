...
public class SessionImpl extends ServiceMethodImpl implements Session, Session2
{
	...
	private class SessionRefreshTask extends TimerTask
	{
		...
		public void run()
		{
			...
			{
				LOGGER.info("refreshing session failed");
			} catch (ImsException e)
			{
				LOGGER.error("refreshing session failed", e);
			}
		}
	}
	...
	public void provisionalResponse(MessageImpl responseMessage)
	{
		...
		LOGGER.info("got response {"+new Integer(statusCode)+"}: {"+ responseMessage.getReasonPhrase()+"}");
		...
		LOGGER.info("Incoming response with SDP - starting media");
	...
	public void methodDelivered(MessageImpl responseMessage)
	{
		...
		LOGGER.info("session response " + statusCode);
	...
	public void methodDeliveryFailed(MessageImpl responseMessage)
	{
		try
		{
			LOGGER.debug("Delivery failed: " + responseMessage.getMethod());
			if (state == STATE_NEGOTIATING)
			...
			if (seh != null)
			{
				...
				if (refresher != null && "uac".equals(refresher))
				{
					if (expireTime > 0)
					{
						...
						if (refreshSeconds > 0)
						{
							LOGGER.info("session update active to refresh after " + refreshSeconds);
							if (timerTask != null)
								timerTask.cancel();
			...
			for (int j = 0; j < remoteMDs.length; j++)
			{
				...
				if (media.getState() == Media.STATE_ACTIVE)
				{
					if (remotePort == 0)
					{
						...
						{
							LOGGER.debug("update media: removal accepted");
							media.setState(Media.STATE_DELETED);
						} else if (media.getUpdateState() == Media.UPDATE_MODIFIED)
						{
							LOGGER.debug("update media: modification rejected");
							media.unprepareUpdate();
						}
					} else
					{
						...
						{
							LOGGER.debug("update media: modification accepted");
							try
							{
				...
				} else if (media.getState() == Media.STATE_PENDING)
				{
					if (remotePort == 0)
					{
						LOGGER.debug("adding media: rejected");
						media.unprepare();
					} else
					{
						try
						{
							LOGGER.debug("adding media: accepted");
	...
	protected void requestReceived(MessageImpl requestMessage)
	{
		...
		{
			LOGGER.debug("Incoming session update");
	...
	private void updateSessionTimer() throws IllegalStateException, ImsException
	{
		...
		LOGGER.debug("session timer update");
	...
	{
		this.state = state;
		LOGGER.debug("Session goes to state: {"+STATES[state - 1]+"}" );
	...
	private SessionDescriptorImpl createOrUpdateMediaList(Message message, ArrayList mediaList)
	{
		...
		{
			LOGGER.info("error in retrieving the sdp content");
			return null;
		}
