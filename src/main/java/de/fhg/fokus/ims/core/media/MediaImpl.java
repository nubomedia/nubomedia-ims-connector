...
{	
	private static Logger logger = LoggerFactory.getLogger(MediaImpl.class);
	...
	public final void prepare() throws MediaPreparationException
	{
		...
		{
			logger.debug("Preparing media with local media descriptor:");
			logger.debug("Media: {"+localMD.getMediaField()+"}");
			logger.debug("Connection: {"+localMD.getConnectionField()+"}");
	...
	public final void complete() throws MediaCompletionException
	{
		if (logger.isDebugEnabled())
		{
			logger.debug("Completing media with remote media descriptor:");
			logger.debug("Media: {"+ remoteMD.getMediaField()+"}");
			
			logger.debug("Connection: {"+ remoteMD.getSessionDescriptor().getConnectionField() + remoteMD.getConnectionField()+"}");
	...
	{
		logger.debug("Closing media");
		onClose();
		setState(Media.STATE_INACTIVE);
		if (localMD != null)
			localMD.setPort(0);
		logger.debug("Closed media");
	}
