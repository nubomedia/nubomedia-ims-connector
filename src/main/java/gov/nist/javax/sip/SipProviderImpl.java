...
public class SipProviderImpl implements javax.sip.SipProvider,
    ...
    {
    	return getNewClientTransaction(request, null, 0, null);
    }
    
	public ClientTransaction getNewClientTransaction(Request request,
    		String remoteUserIP, int toPort)
    		throws TransactionUnavailableException
    {
	  return getNewClientTransaction(request, remoteUserIP, toPort, null);	  
	}    
    
    public ClientTransaction getNewClientTransaction(Request request,
    		String remoteUserIP, int toPort, String proto)
    		throws TransactionUnavailableException 
    {
        if (request == null)
        ...
        if (request.getMethod().equalsIgnoreCase(Request.CANCEL)) {
            ...
            if (ct != null) {
                ...
                try
				{
                	retval.setNextHop(sipStack.getNextHop((SIPRequest) request));
        ...
        if(remoteUserIP!=null && ( remoteUserIP.startsWith("sip:") || remoteUserIP.startsWith("SIP:") ))
        {
			remoteUserIP = SIPUtils.getWithoutSipPrefix(remoteUserIP);
			if(proto==null)
				proto="UDP";
			hop = new HopImpl(remoteUserIP, toPort, "UDP");
		}
