...
public class TCPMessageProcessor extends MessageProcessor {
    ...
    private ArrayList incomingTcpMessageChannels;
	...
	public void start() throws IOException {
		if (super.getListeningPoint().getIsServer() == true) {
			...
			// create a client socket and bind it to the server socket
			InetAddress rAddr = this.getListeningPoint().getRemoteAddr();
			int rPort = this.getListeningPoint().getRemotePort();
			try {
				if (rAddr != null && rPort != 0) {
					clientSocket = sipStack.getNetworkLayer()
							.createSocket(rAddr, rPort, super.getIpAddress(),
									super.getPort());
					// save the socket for future reuse in sendBytesTCP method
					// of IOHandler class
					IOHandler ioh = sipStack.getIOHandler();
					String key = ioh.makeKey(rAddr, rPort);
					ioh.putSocket(key, clientSocket);
				}
			} catch (java.io.IOException ex) {
				if (sipStack.isLoggingEnabled())
					getSIPStack().getStackLogger().logError(
							"Unable to create client socket with parameters:"
    ...
    public void run() {
        ...
        while (this.isRunning) {
            try {
    			...
    			//this is a socket connected with secure client socket of PCSCF
                clientSocket = newsock;
                //save the socket for future reuse in sendBytesTCP method of IOHandler class
                IOHandler ioh = sipStack.getIOHandler();
                String key = ioh.makeKey( clientSocket.getInetAddress(), clientSocket.getPort());
                ioh.putSocket( key, clientSocket );
	...
	public synchronized void stop() {
		if (this.isRunning == true) {
			...
			// RRPN: fix
			for (Iterator incomingMCIterator = incomingTcpMessageChannels
					.iterator(); incomingMCIterator.hasNext();) {
				TCPMessageChannel next = (TCPMessageChannel) incomingMCIterator
						.next();
				next.close();
			}
